...
import { Search, Edit, Trash2, User, Phone, MapPin, Mail, Image as ImageIcon } from 'lucide-react'
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setError(res.message || 'Cập nhật người dùng thất bại')
      }
    } catch (err) {
  ...
  const handleDelete = async (userId) => {
    try {
      ...
      if (res.status === 200) {
        ...
        setError(res.message || 'Xóa người dùng thất bại')
      }
    } catch (err) {
    ...
    <div>
                      ...
                      <div className="flex items-center gap-3">
                        {user.avatarUrl ? (
                          <img
                            ...
                            className="h-10 w-10 rounded-full object-cover border-2 border-opacity-50"
                            onError={(e) => {
      ...
      {!loading && !error && users.length === 0 && (
        <div className={`rounded-lg border p-12 text-center ${isLight
          ...
          <p className={isLight ? 'text-gray-600' : 'text-zinc-400'}>
            {searchName ? 'Thử tìm kiếm với từ khóa khác' : 'Chưa có người dùng nào trong hệ thống'}
          </p>
        </div>
      )}
